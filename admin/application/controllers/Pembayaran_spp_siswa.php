...
class Pembayaran_spp_siswa extends MY_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('Pembayaran_spp_model2');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'pembayaran_spp_siswa/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'pembayaran_spp_siswa/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'pembayaran_spp_siswa/index.html';
            $config['first_url'] = base_url() . 'pembayaran_spp_siswa/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Pembayaran_spp_model2->total_rows($q);
        $pembayaran_spp = $this->Pembayaran_spp_model2->get_all($nis);
        ...
        $this->load->view('pembayaran_spp_list2', $data);
        ...
        echo " <table id='example1' class='table table-hover'>
                 <thead>
                   ...
                   ";
                    $id=$_GET['kode_kelas'];
                    $no=1;
                    $data = $this->db->query("SELECT a.nis,b.nama_siswa,c.biaya_komite,d.biaya_osis from detail_kelas a join siswa b on a.nis=b.nis join komite c on c.kode_kelas=a.kode_kelas join osis d on d.kode_kelas=a.kode_kelas where c.kode_kelas='$id'")->result();
    ...
    public function create_action() 
    {
        echo "test";
            ...
            $kode_kelas=$this->db->query("SELECT a.nis,b.nama_siswa,c.biaya_komite,d.biaya_osis,a.kelas,a.kode_kelas from detail_kelas a join siswa b on a.nis=b.nis join komite c on c.kode_kelas=a.kode_kelas join osis d on d.kode_kelas=a.kode_kelas where c.kode_kelas='$kod'")->result();
