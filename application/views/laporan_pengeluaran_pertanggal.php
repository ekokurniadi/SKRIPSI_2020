         ...
         <?php if($tanggal2 != ''){ ?>
          ...
          <br>
           ...
           <thead>
                   ...
                   <?php $data2=$this->db->query("SELECT a.*,b.* from pengeluaran_barang a join detail_pengeluaran b on a.kode_pengeluaran=b.kode_pengeluaran where a.tanggal between '$tanggal2' and '$tanggal3'")->result();?>
                   <?php $no=1; foreach($data2 as $dt):?>
                    <tr style="text-align:center;">
                    <td><?=$no++;?></td>
                    <td><?=$dt->kode_pengeluaran?></td>
...
function getReport(){
  ...
  if (value.tanggal2 == '' && value.tanggal3 == '') {
    ...
    $('.loader').show();
    $('#btnShow').disabled;
    $("#showReport").attr("src",'<?php echo site_url("laporan_pdf/index5?") ?>cetak='+value.cetak+'&tanggal2='+value.tanggal2+'&tanggal3='+value.tanggal3);
