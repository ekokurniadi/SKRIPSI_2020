...
class Ekskul extends CI_Controller {
    ...
    public function index()
    {
        $nis=$_SESSION['nis'];
        $data=array(
            'data_spp'=>$this->db->query("SELECT * FROM detail_pembayaran where nis='$nis' and status_ekstrakurikuler!='Lunas'")->result(),
        );
    ...
    public function bukti_ekskul($id)
    {
        $data=array(
            'gambar'=>$this->db->query("select * from detail_pembayaran where id='$id'")->row_array(),
        );
        $this->load->view("header");
        $this->load->view("bukti_eks",$data);
        $this->load->view("footer");
    }

    public function hasread()
    {   $id=$this->input->post('id');
        $update=$this->db->query("update detail_pembayaran set flag_eks ='1' where id='$id'");
        if($update){ 
            $this->session->set_flashdata('message', '
				...
				<div class="toast-body" style="color:black;">
                   Pesan sudah di baca
					<br/>
					Untuk melihat history pembayaran bisa lihat pada menu Ekstrakurikuler -> History Pembayaran Ekstrakurikuler
				</div>
			</div>');
